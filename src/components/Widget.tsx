...
import useWidgetStore, { WidgetState, WidgetStore } from '@/lib/store'
...
const getParams = (store: WidgetStore) => ({
	reset: store.reset,
	theme: store.theme,
	query: store.query,
	state: store.state,
...
const Widget: FC = () => {
	...
	const {
		...
		setReferences,
		overwriteAnswer,
	} = useWidgetStore(getParams)

	const { data: project } = useSWRImmutable<WidgetData>(
		// 'https://api.usevogal.com/widget',
		'https://usevogal.com/api/widget',
	...
	useEffect(() => {
		if (!project) return
		console.groupCollapsed(`${project.name}' é uma assistente com IA que é criada por [Name]`)
		console.log(`__

				Vogal permite adicionar uma IA 
				  assistente da sua documentação minutos.

				Visite https://usevogal.com para saber mais.
	...
	const toggleOpen = useCallback(() => {
		if (state != WidgetState.Splash) return reset()

		setState(WidgetState.UserTyping)
	}, [])
	...
	const askQuestion = useCallback(
		async (event: FormEvent<HTMLFormElement>) => {
			...
			await fetchEventSource(`https://api.usevogal.com/widget/stream`, {
				method: 'POST',
			...
			<div className={classNames(theme == 'dark' && 'dark', 'fixed bottom-5 right-5 pl-5 md:pl-0 z-50')}>
				...
				{media == 'desktop' || showOnMobile ? (
					<motion.div
						...
						animate={{
							y: 0,
							width: state == WidgetState.Finished ? (media == 'mobile' ? 'auto' : '24rem') : '18rem',
						}}
						transition={{
							width: { type: state == WidgetState.Finished ? 'spring' : 'tween', duration: 0.15 },
						}}
								...
								{state == WidgetState.Finished ? (
									...
									<div className="p-3">
										<div className="flex items-center gap-2">
											...
											>
												<motion.img
													title="Desenvolvido por usevogal.com"
													layoutId="vogalImg"
									...
									<div className="flex items-center gap-2 p-3">
										...
										<a target="_blank" href="usevogal.com" className="relative h-12 w-12 shrink-0">
											<motion.img
												title="Desenvolvido por usevogal.com"
												layoutId="vogalImg"
												src={project.image_url}
					...
					<motion.button onClick={() => setMobileShow(true)} className="relative h-12 w-12 shrink-0">
						<motion.img layoutId="vogalImg" src={project.image_url} alt="avatar" className="rounded-full" />
					</motion.button>
				)}
			</div>
		</root.div>
